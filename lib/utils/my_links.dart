final Uri urlLinkedin = Uri.parse('https://www.linkedin.com/in/aribimajdi');
final Uri urlGithub = Uri.parse('https://github.com/majdideveloper');
final Uri urlYoutube =
    Uri.parse('https://www.youtube.com/channel/UCuX1Dc5Bz3rrER49IZk8-Cg');
final Uri urlInstagram = Uri.parse('https://www.instagram.com/dev.majdi/');
final Uri urlTiktok = Uri.parse('https://www.tiktok.com/@majdideveloper');
final Uri urlEmail = Uri.parse('mailto:majdiaribi@outlook.com?');
final Uri urlCv = Uri.parse(
    'https://drive.google.com/file/d/1A8QNia1QN2j_E6qXwXOrrmAHRDWN1u2o/view?usp=sharing');

final Uri urlGithubBoxTime =
    Uri.parse("https://github.com/majdideveloper/box_time");
final Uri urlGithubHolbegram = Uri.parse(
    "https://github.com/majdideveloper/holbertonschool-holbegram/tree/main/holbegram");
final Uri urlGithubEcommerce =
    Uri.parse("https://github.com/majdideveloper/flutter_ecommerce_getx");
final Uri urlGithubChatgbt =
    Uri.parse("https://github.com/majdideveloper/my_own_chatGBT");
final Uri urlGithubMyPortfilo =
    Uri.parse("https://github.com/majdideveloper/portfolio_majdouch");

final Uri urlGithubKoofin =
    Uri.parse("https://github.com/majdideveloper/api_koofin");
final Uri urlGithubSimpleShell =
    Uri.parse("https://github.com/majdideveloper/holbertonschool-simple_shell");
final Uri urlGithubRayek = Uri.parse("https://github.com/majdideveloper/rayek");

final Uri urlPreviewKoofin = Uri.parse(
    "https://medium.com/@5004_52812/hack-4-track-hackathon-69e013355e9b");
final Uri urlPreviewRayek = Uri.parse("https://ouhana.wixsite.com/rayek");
